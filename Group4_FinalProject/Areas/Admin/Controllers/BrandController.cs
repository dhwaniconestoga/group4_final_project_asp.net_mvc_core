...
namespace Group4_FinalProject.Areas.Admin.Controllers
{
    ...
    public class BrandController : Controller
    {
        ...
        public IActionResult Add(Brand brand, string operation)
        {
            ...
            {
                validate.CheckBrand(brand.Name, operation, data);
                if (!validate.IsValid)
                {
                    ModelState.AddModelError(nameof(brand.Name), validate.ErrorMessage);
                }
            }
            ...
            if (ModelState.IsValid)
            {
                ...
                data.Save();
                validate.ClearBrand();
                TempData["message"] = $"{brand.FullName} added to Brands.";
                return RedirectToAction("Index");
            }
            else
            {
